...
class EvaluationService {
     ...
     */
    static async getQuestionsByCareer(carreraId) {
        try {
            return await supabase_only_1.SupabaseDB.getQuestionsByCareer(carreraId);
        }
        catch (error) {
    ...
    static async getAllActiveQuestions() {
        try {
            return await supabase_only_1.SupabaseDB.getQuestionsWithCategories();
        }
        catch (error) {
     ...
     * @returns Array de preguntas de evaluación
     */
    static async getBasicQuestions() {
        try {
            return await supabase_only_1.SupabaseDB.getEvaluationQuestions();
        }
        catch (error) {
    ...
    static async createQuestion(preguntaData) {
        try {
            const { data, error } = await supabase_only_1.SupabaseDB.supabaseAdmin
     ...
     */
    static async updateQuestion(preguntaId, updateData) {
        try {
            const { data, error } = await supabase_only_1.SupabaseDB.supabaseAdmin
                .from('preguntas_evaluacion')
                .update(updateData)
     ...
     */
    static async deactivateQuestion(preguntaId) {
        try {
            const { error } = await supabase_only_1.SupabaseDB.supabaseAdmin
                .from('preguntas_evaluacion')
                .update({ activa: false })
                .eq('id', preguntaId);
            if (error)
     ...
     */
    static async getQuestionsByCategoryAndCareer(categoriaId, carreraId) {
        try {
