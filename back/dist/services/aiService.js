...
const genAI = process.env.GOOGLE_GEMINI_API_KEY
    ? new generative_ai_1.GoogleGenerativeAI(process.env.GOOGLE_GEMINI_API_KEY)
    : null;
...
function extractTopKeywords(texts, maxTerms = 10) {
    ...
    for (const raw of texts) {
        ...
        for (let i = 0; i < tokens.length - 1; i++) {
            ...
            if (!a || !b)
                continue;
            if (SPANISH_STOPWORDS.has(a) || SPANISH_STOPWORDS.has(b))
                continue;
...
function generateSmartSummary(responses) {
    if (responses.length === 0)
        return 'No hay respuestas para resumir.';
    ...
    responses.forEach(r => {
        ...
        negativeWords.forEach(w => { if (lower.includes(w))
            negativeCount++; });
        neutralWords.forEach(w => { if (lower.includes(w))
            mentions[w] = (mentions[w] || 0) + 1; });
    });
...
class AiService {
    static async summarizeOpenResponses(responses, context = 'profesor') {
