...
async function seed() {
    ...
    try {
        ...
        const { data: facultad, error: errFacultad } = await supabase_only_1.supabaseAdmin
            .from('facultades')
            .insert({
        ...
        if (errFacultad) {
            if (errFacultad.code === '23505') {
                ...
                const { data: f } = await supabase_only_1.supabaseAdmin.from('facultades').select('id').eq('codigo', 'F-ING').single();
                facultadId = f?.id ?? 1;
            }
            else {
                throw errFacultad;
        ...
        const { data: carrera, error: errCarrera } = await supabase_only_1.supabaseAdmin
            .from('carreras')
            .insert({
        ...
        if (errCarrera) {
            if (errCarrera.code === '23505' || String(errCarrera.message).includes('duplicate')) {
                ...
                const { data: c } = await supabase_only_1.supabaseAdmin.from('carreras').select('id').eq('nombre', 'Ingeniería de Sistemas').limit(1).single();
                carreraId = c?.id ?? 1;
            }
            else {
                throw errCarrera;
        ...
        const { data: periodo, error: errPeriodo } = await supabase_only_1.supabaseAdmin
        ...
        const { data: existe } = await supabase_only_1.supabaseAdmin
            .from('usuarios')
            .select('id')
            .eq('email', email)
            .maybeSingle();
        ...
        const hashedPassword = await bcrypt_1.default.hash(passwordPlano, 10);
        const { data: usuario, error: errUsuario } = await supabase_only_1.supabaseAdmin
            .from('usuarios')
            .insert({
            email,
        ...
        const { data: estudianteExistente } = await supabase_only_1.supabaseAdmin
            .from('estudiantes')
            .select('id')
            .eq('usuario_id', usuarioId)
            .maybeSingle();
        if (estudianteExistente) {
            await supabase_only_1.supabaseAdmin
                .from('estudiantes')
                .update({
            ...
            await supabase_only_1.supabaseAdmin.from('estudiantes').insert({
        ...
        const { error: errRol } = await supabase_only_1.supabaseAdmin.from('usuario_roles').upsert({
            usuario_id: usuarioId,
            rol: 'estudiante',
            activo: true,
