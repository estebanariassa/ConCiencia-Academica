    ...
    try {
      ...
      `)
        ...
        const evaluacionesPorCurso = evaluaciones?.reduce((acc, evaluacion) => {
            const cursoData = evaluacion.cursos;
        ...
        Object.values(evaluacionesPorCurso).forEach((curso) => {
            const suma = curso.evaluaciones.reduce((sum, evaluacion) => sum + (evaluacion.calificacion_general || 0), 0);
        ...
        const evaluacionesRecientes = evaluaciones
            ...
            ?.map(evaluacion => {
            const cursoData = evaluacion.cursos;
            const grupoData = evaluacion.grupos;
            return {
                id: evaluacion.id,
...
router.post('/evaluations', auth_1.authenticateToken, async (req, res) => {
    try {
        const user = req.user;
        const { teacherId, courseId, groupId, answers, overallRating, comments } = req.body;
    ...
    try {
      ...
      `)
        ...
        if (cursoIds.length > 0) {
            ...
            const { data: todosLosCursos, error: todosLosCursosErr } = await supabase_only_1.SupabaseDB.supabaseAdmin
                .from('cursos')
                .select('id, nombre, codigo, carrera_id')
