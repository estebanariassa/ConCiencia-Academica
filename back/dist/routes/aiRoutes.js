...
router.post('/summarize', auth_1.authenticateToken, (0, auth_1.requireRole)(['docente', 'profesor', 'coordinador', 'decano', 'admin']), async (req, res) => {
    try {
        const { texts } = req.body;
        if (!Array.isArray(texts) || texts.length === 0) {
            return res.status(400).json({ error: 'Se requiere un array no vacío en "texts"' });
        }
        const result = await aiService_1.AiService.summarizeOpenResponses(texts);
        res.json(result);
    }
    catch (error) {
...
async function fetchOpenTextsByFilters(filters) {
    const { SupabaseDB } = await Promise.resolve().then(() => __importStar(require('../config/supabase-only')));
    console.log('\n🔍 [AI Routes] ========================================');
...
router.get('/summarize/by-professor', auth_1.authenticateToken, (0, auth_1.requireRole)(['docente', 'profesor', 'coordinador', 'decano', 'admin']), async (req, res) => {
    try {
        ...
        const filters = { profesor_id: String(profesor_id) };
        ...
        if (grupo_id)
            filters.grupo_id = Number(grupo_id);
...
ORDER BY e.id, re.id;`;
        ...
        const result = await aiService_1.AiService.summarizeOpenResponses(texts, 'profesor');
...
router.get('/summarize/by-career', auth_1.authenticateToken, (0, auth_1.requireRole)(['coordinador', 'decano', 'admin']), async (req, res) => {
    try {
...
ORDER BY evaluacion_id, respuesta_id;`;
        ...
        const texts = respuestas
            .map((r) => String(r.respuesta_texto).trim())
            .filter((texto) => texto.length > 0 && texto.length >= 3); // >= 3 coincide con el SQL
        ...
        const result = await aiService_1.AiService.summarizeOpenResponses(texts, 'coordinador');
...
router.get('/summarize/by-faculty', auth_1.authenticateToken, (0, auth_1.requireRole)(['decano', 'admin']), async (req, res) => {
    try {
        const { periodo_id } = req.query;
        const { SupabaseDB } = await Promise.resolve().then(() => __importStar(require('../config/supabase-only')));
        ...
        const texts = (respuestas || [])
            .map((r) => String(r.respuesta_texto || '').trim())
            .filter((texto) => texto.length > 0 && texto.length > 3);
...
ORDER BY evaluacion_id, id;`;
        ...
        const result = await aiService_1.AiService.summarizeOpenResponses(texts, 'decano');
