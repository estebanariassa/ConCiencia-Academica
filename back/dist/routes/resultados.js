...
router.get('/', auth_1.authenticateToken, (0, auth_1.requireRole)(['profesor', 'coordinador', 'admin']), async (req, res) => {
    try {
        ...
        if (periodo_id) {
            filters.periodo_id = parseInt(periodo_id);
        }
        if (grupo_id) {
            filters.grupo_id = parseInt(grupo_id);
        }
        // Obtener evaluaciones con relaciones
        const { data: evaluaciones, error } = await supabase_only_1.SupabaseDB.supabaseAdmin
            .from('evaluaciones')
            .select(`
        *,
...
router.get('/estadisticas', auth_1.authenticateToken, (0, auth_1.requireRole)(['profesor', 'coordinador', 'admin']), async (req, res) => {
    try {
        const { periodo_id, grupo_id } = req.query;
        let filters = {};
        if (req.user.tipo_usuario === 'profesor') {
